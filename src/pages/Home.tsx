...
function Home() {
  ...
  function searchAddress(address:any) {
    console.log(address)
    console.log("YM",ymapApi)
    ymapApi?.geocode(`Ижевск, ${address}`, {
      results: 1,
    }).then((res:any )=> {
    ...
    <div className=" mt-5 max-w-screen-md mx-auto flex flex-col">
      ...
      <div className="flex items-center mb-10">
        ...
        <div className="w-2/3 h-10">
          ...
          />
          {showError ? <span className="text-red-600 pl-2 h-5">{errorMessage}</span>:<span className="h-5"> </span>} 
        </div>
      </div>
      <div className="flex">
        ...
        {taxi ? (
          ...
          <div className="h-32">Такси не найдено</div>
        )}
      </div>
      ...
      ?<button disabled className="bg-gray-400 border border-gray-100 rounded-md  px-4 py-1 shadow-md max-w-min self-center">
      Заказать
    </button>
      :<button type="submit" className="bg-green-600 border border-gray-300 rounded-md hover:bg-green-500 px-4 py-1 shadow-md max-w-min self-center">
        Заказать
      </button>
    }
